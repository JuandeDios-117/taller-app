...
app.get('/api/usuarios', async (req, res) => {
    try {
        await db.execute(`
            ...
            SELECT id, 0, 0 FROM usuarios
        `);
        const rows = await queryRows("SELECT id, nombre, usuario, COALESCE(avatar, '/logo.png') as avatar, COALESCE(rol, 'empleado') as rol, COALESCE(comision_porcentaje, 30) as comision_porcentaje, COALESCE(created_at, CURRENT_TIMESTAMP) as created_at FROM usuarios ORDER BY id ASC");
